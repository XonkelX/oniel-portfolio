...
1. Reticle #278 persisted learned routes across crawls and ordinary navigation. The change added bounded and batched persistence plus regression coverage across ten files. The maintainer called it the strongest pull request in the queue, requested two growth-related corrections, confirmed both were addressed, and merged it after the complete CI, E2E, desktop, install, and cross-platform gates passed.
...
5. Apache Fineract Backoffice UI #475 migrated fifteen client-area test files to Vitest. Maintainer approval preceded merge; unit, build, formatting, security, CodeQL, and extensive mocked and real-backend E2E gates passed.
